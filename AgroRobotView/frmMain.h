...
#pragma region Windows Form Designer generated code
		   ...
		   {
			   // (MANTÉN TU CÓDIGO DE DISEÑO ORIGINAL AQUÍ)
			   // Para no saturar el mensaje, asumo que el diseño visual 
			   // (colores, botones, panel) lo tienes intacto en tu archivo.
...
#pragma endregion
	...
	{
		// RECOMENDACIÓN: MANTÉN EL TRIM() PARA EVITAR ERRORES DE ESPACIOS
		...
		for each(Usuario ^ u in lista) {
			// LOGICA BÁSICA ORIGINAL (Con limpieza de espacios por seguridad)
			if (u->GetNombre()->Trim() == usuarioIngresado &&
				u->GetContrasenha()->Trim() == contrasenhaIngresada)
			{
				...
				// Abrir Menú Original (Sin bucles complejos)
				frmMenu^ menu = gcnew frmMenu(u);
				menu->ShowDialog();

				this->Close(); // Al cerrar menú, cerramos todo
				return;
			}
		}
	...
	{
		if (chkVerContrasenha->Checked) txtPassword->UseSystemPasswordChar = false;
